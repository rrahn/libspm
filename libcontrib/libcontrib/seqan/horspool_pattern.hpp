...
namespace jst::contrib
{
    ...
    class horspool_pattern
    {
    private:

        using pattern_t = seqan::Pattern<needle_t, seqan::Horspool>;
        struct operation;
    ...
    public:
        ...
        horspool_pattern() = delete;
        template <typename _needle_t>
            requires std::constructible_from<pattern_t, _needle_t>
        explicit horspool_pattern(_needle_t && needle) : _pattern{(_needle_t &&) (needle)}
        {
        }
    ...
    template <typename needle_t>
    horspool_pattern(needle_t &&) -> horspool_pattern<std::remove_reference_t<needle_t>>;

    template <typename needle_t>
    class horspool_pattern<needle_t>::operation : public pattern_operation<operation, pattern_t> // run as CRTP?
    {
    private:
    ...
    private:
        ...
        constexpr friend void tag_invoke(std::tag_t<jst::contrib::set_up>, operation & me, finder_t & finder)
            noexcept
        {
            if (!me._find_first) // we overwrite the behaviour of the seqan::find method implemented for Horspool.
            {
