...
namespace libio
{
    ...
    public:
        unformatted_file(std::filesystem::path &&file_path, format_t format = {}) : _format{std::move(format)}
        {
        ...
        // What with header information?
        // Returns an unformatted record!
        auto read_record() noexcept(is_nothrow_tag_invocable_v<tag_t<libio::get_record>, format_t &, stream_t &>)
            -> tag_invoke_result_t<tag_t<libio::get_record>, format_t &, stream_t &>
        {
            assert(_stream != nullptr);

            // Check applicability of a header.
            return libio::get_record(_format, _stream->iter);
        }
    ...
    template <typename format_t>
    unformatted_file(std::filesystem::path &&, format_t) -> unformatted_file<format_t>;
} // namesapce libio
