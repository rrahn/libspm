...
public:
    ...
    constexpr void operator()(haystack_t && haystack, callback_t && on_hit) noexcept
    {
        ...
        {
            if (_state_manager.state() > 0)  // Advance the haystack iterator until we can compare again.
            {
                --_state_manager.state();
                continue;
            }

            assert(_state_manager.state() == 0);

            auto base_iterator = it.base(); // Returns the original iterator to the JD!

            int32_t pattern_position = pattern_size;
            while (--pattern_position >= 0 && _pattern[pattern_position] == *base_iterator)
                --base_iterator;
