...
namespace libjst
{
    ...
    namespace _searcher_factory
    {
        ...
        private:
            ...
            {
                using jst_node_t = jst_node<std::remove_cvref_t<jst_t>>;
                using tree_t = lazy_tree<jst_node_t>;

                if constexpr (libjst::is_resumable_v<std::remove_cvref_t<pattern_t>>)
                    return tree_t{_jst, libjst::window_size(_pattern), subscriber};
                else
                    return tree_t{_jst, libjst::window_size(_pattern)};
            }
