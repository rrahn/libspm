...
namespace libjst
{
    ...
    public:
        ...
        sequence_strategy_node_only(source_sequence_t && source) : base_t{(source_sequence_t &&) source}
        {}

        // returns full sequence, but interface is protected
        auto sequence() const noexcept
        {
            return base_t::journal().sequence() | seqan3::views::slice(base_t::begin_position(), base_t::end_position());
        }
    };
}  // namespace libjst
