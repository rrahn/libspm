...
namespace libjst
{
    ...
    private:
        ...
        position_value_type _min_position{};

    public:

        breakend_site_min() = default;

        explicit constexpr breakend_site_min(wrapped_breakend_site_t breakend_site,
                                              position_value_type min_position = std::numeric_limits<position_value_type>::lowest()) :
    ...
    private:
        ...
        tag_invoke(libjst::tag_t<libjst::position>, breakend_site_min const & me) noexcept
        {
            return std::max(libjst::position(me._wrappee), me._min_position);
        }

        constexpr friend bool operator==(breakend_site_min const &, breakend_site_min const &) noexcept = default;
    };
